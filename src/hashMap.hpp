...
class hashMap {
	...
	int numKeys;
	int mapSize;
	bool hash1; // if true, first hash function used, otherwise second hash function is used
	bool coll1; //if true, first collision method used, otherwise second collision method is used.
...
public:
	...
	int getIndex(string key); // uses calcHash and reHash to calculate and return the index of where the keyword k should be inserted into the map array
	int calcHash(string key);  // hash function
	int calcHash2(string key);  // hash function 2

	int getClosestPrime(int mapSize); // I used a binary search on an array of prime numbers to find the closest prime to double the map Size, and then set mapSize to that new prime.  You can include as one of the fields an array of prime numbers, or you can write a function that calculates the next prime number.  Whichever you prefer.
